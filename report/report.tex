...
Figure \ref{fig:returns} shows training performance. The vertical dashed line marks the warm start boundary at 30,000 steps. The 90th percentile configuration demonstrates steady convergence toward baseline performance throughout training. In contrast, the 99th percentile configuration completely fails to learn, with returns remaining near zero, demonstrating that excessive sparsification discards critical gradient information required for effective policy learning.

\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{figures/episodic_returns.pdf}}
\caption{Training performance comparison. EMA-smoothed episodic returns show 90th percentile sparsification converges toward baseline performance, while 99th percentile fails to learn. The dashed vertical line indicates warm start completion at 30,000 steps.}
\label{fig:returns}
\end{figure}

Figure \ref{fig:network} presents cumulative data transfer. The baseline transmits 133.86 GB total, while the 90th percentile configuration achieves 34\% reduction (88.38 GB)—saving 45.48 GB. The 90th percentile configuration strikes a favorable balance: converging toward baseline performance while achieving substantial bandwidth savings for bandwidth-constrained deployments.

\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{figures/network_transfer.pdf}}
\caption{Cumulative network transfer comparison. Gradient accumulation with 90th percentile threshold reduces data transfer by 34\%, saving 45.48 GB over 400,000 training steps.}
\label{fig:network}
\end{figure}
...
To further characterize final policy quality, we evaluated each trained checkpoint over 100 independent environment episodes with a fixed random seed, and compared the resulting return distributions. Each checkpoint was loaded after training completed and rolled out in CarRacing-v3 without any further updates — providing an unbiased estimate of the learned policy's generalization. Figure \ref{fig:boxplot} shows box plots of episodic returns for the baseline and 90th percentile configurations. The baseline achieves a mean return of 398.97 ($\pm$226.58) with a median of 376.68, while gradient accumulation achieves a mean of 305.32 ($\pm$161.73) with a median of 283.18. Notably, the interquartile ranges substantially overlap—the gradient accumulation Q1–Q3 range of [190, 416] versus the baseline range of [284, 520]—and the 90th percentile model achieves 76.5\% of baseline mean performance. The tighter standard deviation under compression (161.73 vs. 226.58) suggests the compressed model produces more consistent, if somewhat lower, returns. These results indicate that gradient accumulation does not catastrophically degrade policy quality, and the performance gap may narrow further with additional training.

\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{figures/performance_comparison.pdf}}
\caption{Evaluation return distributions over 100 independent episodes per checkpoint (no further training). Box plots show median, IQR, and whiskers (1.5$\times$IQR). Gradient accumulation (90th percentile) achieves 76.5\% of baseline mean return with overlapping IQRs, indicating the performance gap is moderate rather than catastrophic.}
\label{fig:boxplot}
\end{figure}
